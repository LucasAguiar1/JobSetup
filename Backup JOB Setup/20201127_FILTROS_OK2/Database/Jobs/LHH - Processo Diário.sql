...
/****** Object:  Job [LHH - Processo Diário]    Script Date: 26/07/2019 17:11:03 ******/
BEGIN TRANSACTION
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'LHH - Processo Diário', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Processamento diário do sistema Libras Homem Hora', 
		@category_name=N'[Uncategorized (Local)]', 
		@owner_login_name=N'BSLA\salvatopaulo', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [LHH - Processa Experiência]    Script Date: 26/07/2019 17:11:11 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'LHH - Processa Experiência', 
		@step_id=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'LHH Diário', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
