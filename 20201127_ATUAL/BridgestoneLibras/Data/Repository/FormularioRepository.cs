...
namespace LHH.Data.Repository
{
    public class FormularioRepository : SpecRepository<TB_FORMULARIO>
    {
        ...
        //public List<TB_FORMULARIO> ConsultaGridOrdemFormulario(TB_ORDEM_FORMULARIO ordemFormulario)
        //{
        //    try
        //    {
        //        List<TB_FORMULARIO> listFormulario = new List<TB_FORMULARIO>();
        ...
        public List<TB_FORMULARIO> OrdemPriodidade(TB_FORMULARIO form)
        {
            try
            {
                List<TB_FORMULARIO> listFormulario = new List<TB_FORMULARIO>();
                ...
                using (var command = Context.Database.GetDbConnection().CreateCommand())
                {
                    command.CommandText = "sp_consulta_formulario_Prioridade";
                    command.CommandType = CommandType.StoredProcedure;
        ...
        public List<TB_FORMULARIO> Consultar(TB_FORMULARIO form)
        {
            try
            {
                ...
                using (var command = Context.Database.GetDbConnection().CreateCommand())
                {
                    ...
                    if (form.id_parteMaquina == 0)
                    {
                        command.Parameters.Add(new SqlParameter("@id_parteMaquina", null));
                    }
                    else
                    {
                        command.Parameters.Add(new SqlParameter("@id_parteMaquina", form.id_parteMaquina));
                    }
