...
namespace LHH.Data.Repository
{
    public class MaquinaRepository : SpecRepository<TB_MAQUINA>
    {
                    ...
                    using (var result = command.ExecuteReader())
                    {
                        if (result.HasRows)
                        {
                            ...
                            while (result.Read())
                            {
                                ...
                                tbMaquina.i_Pk_Maquina = result["i_Pk_Maquina"].ToString() == "" ? 0 :  Convert.ToInt32(result["i_Pk_Maquina"].ToString());
                                tbMaquina.codigoTess = result["codigoTess"].ToString() == null ? string.Empty : result["codigoTess"].ToString();
        ...
        public List<TB_MAQUINA> Consulta(TB_MAQUINA maquina)
        {
            try
            {
                List<TB_MAQUINA> lMaquina = new List<TB_MAQUINA>();
                    ...
                    using (var result = command.ExecuteReader())
                    {
                        if (result.HasRows)
                        {
                            ...
                            while (result.Read())
                            {
                                ...
                                tbMaquina.departamento = result["departamento"].ToString().Trim();
                                tbMaquina.id_departamento = Convert.ToInt32(result["id_departamento"].ToString());
                                tbMaquina.codigoTess = result["codigoTess"].ToString();
                                lMaquina.Add(tbMaquina);
                            }
                        }
                    }
        ...
        public TB_MAQUINA Alterar(TB_MAQUINA maquina)
        {
            ...
            try
            {
                using (var command = Context.Database.GetDbConnection().CreateCommand())
                {
                    ...
                    command.Parameters.Add(new SqlParameter("@codigoTess", maquina.codigoTess));
                    
                    command.Parameters.Add(new SqlParameter("@id", maquina.id));
                    if(maquina.i_Pk_Maquina == 0)
                    {
                        command.Parameters.Add(new SqlParameter("@i_Pk_Maquina", DBNull.Value));
                    }
                    else
                    {
                        command.Parameters.Add(new SqlParameter("@i_Pk_Maquina", maquina.i_Pk_Maquina));
                    }
                    command.Parameters.Add(new SqlParameter("@vc_Descripcion", maquina.vc_Descripcion));
