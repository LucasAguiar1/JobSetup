        ...
        public List<TB_PERGUNTA> Consultar(TB_PERGUNTA pergunta)
        {
            ...
            try
            {
                ...
                using (var command = Context.Database.GetDbConnection().CreateCommand())
                {
                    ...
                    //if (pergunta.id_departamento == 0)
                    //{
                    //    command.Parameters.Add(new SqlParameter("@id_departamento", DBNull.Value));
                    //}
                    //else
                    //{
                    //    command.Parameters.Add(new SqlParameter("@id_departamento", pergunta.id_departamento));
                    //}

                    if (pergunta.id_formulario == 0)
                    {
                        command.Parameters.Add(new SqlParameter("@id_Formulario", DBNull.Value));
                    }
                    else
                    {
